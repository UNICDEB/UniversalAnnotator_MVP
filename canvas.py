...
from PySide6.QtGui import (
    QAction, QPixmap, QImage, QPainterPath, QMouseEvent,
...
class AnnotCanvas(QGraphicsView):
    def __init__(self, state):
        ...
        self.setRenderHints(self.renderHints() | QPainter.Antialiasing)
        self.pix = None
        self.mode = "select"   # rect, polygon, circle, line, keypoint, select/edit
        self.temp_points = []
        ...
        # Colors per shape type
        self.shape_colors = {
            "rect": QColor("red"),
            "circle": QColor("blue"),
            "line": QColor("green"),
            "polygon": QColor("orange"),
            "polyline": QColor("purple"),
            "keypoint": QColor("cyan"),
        }
    ...
    # ----------------------------
    # Context menu (mode switch + edit/delete)
    # ----------------------------
    def contextMenuEvent(self, event):
        menu = QMenu(self)
        actions = {
    ...
    def mousePressEvent(self, e: QMouseEvent):
        ...
        if self.edit_mode:  # Selecting shapes in edit mode
            items = self.scene.items(e.pos())
            if items and items[0] != self.pix:
    ...
    def mouseMoveEvent(self, e: QMouseEvent):
        if not self.temp_points or self.edit_mode:
            return
        scene_pos = self.mapToScene(e.pos())
        if self.preview_item:
            self.scene.removeItem(self.preview_item)
    ...
    def mouseReleaseEvent(self, e: QMouseEvent):
        if self.edit_mode:
            return  # no class chooser in edit mode
    ...
    def _draw_shape(self, sh: Shape):
        color = self.shape_colors.get(sh.type, QColor("magenta"))
        pen = QPen(color, 3)
